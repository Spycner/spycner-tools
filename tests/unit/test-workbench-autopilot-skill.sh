...
# Test 12: Plugin manifests at 0.12.0
echo "Test 12: Plugin manifests at 0.12.0..."
...
if jq -e '.version == "0.12.0"' "$CCM" >/dev/null && jq -e '.version == "0.12.0"' "$CXM" >/dev/null; then
    echo "  [PASS] both plugin manifests at 0.12.0"
else
    echo "  [FAIL] plugin manifests not at 0.12.0"
    exit 1
fi
echo ""

# Test 13: Marketplace entries at 0.12.0
...
if jq -e '.plugins[] | select(.name == "workbench") | .version == "0.12.0"' "$MP" >/dev/null; then
    ...
    echo "  [FAIL] Claude marketplace workbench not at 0.12.0"
    exit 1
fi
echo ""
