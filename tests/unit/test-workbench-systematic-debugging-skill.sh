...
[ "$CC_VER" = "0.14.0" ] || fail "claude-plugin version is $CC_VER, expected 0.14.0"
[ "$CDX_VER" = "0.14.0" ] || fail "codex-plugin version is $CDX_VER, expected 0.14.0"
pass "plugin manifests at 0.14.0"

# 14. Claude marketplace entry at 0.14.0 (codex marketplace does not carry a per-plugin version field).
CC_MP_VER="$(jq -r '.plugins[] | select(.name=="workbench") | .version' "$ROOT/.claude-plugin/marketplace.json")"
[ "$CC_MP_VER" = "0.14.0" ] || fail "claude marketplace workbench version is $CC_MP_VER, expected 0.14.0"
pass "claude marketplace at 0.14.0"
